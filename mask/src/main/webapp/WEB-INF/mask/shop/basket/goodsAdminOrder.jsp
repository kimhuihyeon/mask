							...
							<table
								...
								<c:when test="${fn:length(list)>0 && list.get(0).ORDER_NUM!=null}">
									...
									<tr class="gradeA even" role="row">
										...
										<th style="text-align:center;vertical-align:middle;">${row.GOODS_CATEGORY2}</th>
							...
							<div style="text-align:center;">
								<div id="dataTables-example_filter" class="dataTables_filter">
									<select class="form-control" name="searchNum" id="searchName">
										...
										<input class="form-control" type="text" name="isSearch" id="SearchValue"/>
										<span>
 ...
 <script type="text/javascript">
    ...
    document.querySelector('#order_pay_search').addEventListener('change',(event)=>{
        ...
        $.ajax({
          ...
          success : function test(data) {
              $("#load_fn").load("${root}admin/orderList.mk ","Select_ORDER_PAY="+value);
      	  }
    ...
    document.querySelector('#Select_AORDER_STATE').addEventListener('change',(event)=>{
        ...
        $.ajax({
          ...
          success : function test(data) {
              $("#load_fn").load("${root}admin/orderList.mk","Select_AORDER_STATE="+value);
      	}
        })
    })
    ...
    if(document.getElementById("but")){
    	...
    	document.querySelector('#but').addEventListener('click',()=>{
    		...
    		const elementV= document.querySelector('#aorder_state').value;
 	   		console.log(elementV);
 	   		const elementOrderNum=$('#but').parent('td').parent('tr').children().first().text();
 	   		
 	   		$.ajax({
 	   			type:"post",
 	   			url:"${root}admin/orderUpdate.mk?AORDER_STATE="+elementV+"&&ORDER_NUM="+elementOrderNum,
 	   			success:function(data){
 	   				alert('변경되었습니다.');
 	   				$("#load_fn").load("${root}admin/orderList.mk","Select_AORDER_STATE="+elementV);
 	   			}
    ...
    document.querySelector('#searchBtn').addEventListener('click',()=>{
        ...
        $.ajax({
        	type: "get",
        	url:"${root}admin/orderSearch.mk?name="+element+"&&value="+value,
        	success:function(data){
        		console.log('성공');
        	}
        })
